...
async function ingestCompletedTask(taskId, statusPayload) {
    ...
    const existingMeta = getStagingMeta(taskId);
    ...
    fs.writeFileSync(path.join(dir, 'meta.json'), JSON.stringify({
        taskId, owner: existingMeta && existingMeta.owner, ingested_at: Date.now(), tracks: ingested, status: statusPayload && statusPayload.status,
    }, null, 2));
    return ingested;
}
...
function setStagingOwner(taskId, username) {
    ensureStaging();
    const dir = path.join(STAGING_DIR, taskId);
    fs.mkdirSync(dir, { recursive: true });
    const meta = getStagingMeta(taskId) || {};
    fs.writeFileSync(path.join(dir, 'meta.json'), JSON.stringify({
        ...meta, taskId, owner: username,
    }, null, 2));
}
