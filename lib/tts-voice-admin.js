...
async function fetchYouTubeSource(url) {
    ...
    if (r.code !== 0) {
        ...
        // Classify common yt-dlp failure modes into user-facing reasons so the
        // UI shows something helpful instead of a 5-kB traceback that CF's
        // tunnel sometimes wraps into a 502.
        let short, status = 502;
        if (/Sign in to confirm your age/i.test(raw)) { short = 'Age-restricted video — yt-dlp needs YouTube cookies. Use a different URL or run the trainer path which supports auth.'; status = 422; }
        else if (/Private video/i.test(raw))         { short = 'Video is private.'; status = 422; }
        else if (/Video unavailable/i.test(raw))     { short = 'Video unavailable (removed, region-blocked, or member-only).'; status = 422; }
        else if (/members-only/i.test(raw))          { short = 'Channel members-only video.'; status = 422; }
        else if (/This live event will begin/i.test(raw)) { short = 'Upcoming live stream — not available yet.'; status = 422; }
        else if (/HTTP Error 429/i.test(raw))        { short = 'YouTube rate-limited us — try again in a few minutes.'; status = 429; }
        else {
            // Fall back to the last non-empty line, truncated to 300 chars. Strip
            // newlines so the Node → CF tunnel doesn't choke on multi-line bodies.
            const last = raw.split('\n').filter(l => l.trim()).slice(-1)[0] || 'unknown error';
            short = last.replace(/\s+/g, ' ').slice(0, 300);
        }
        throw Object.assign(new Error('yt-dlp: ' + short), { status });
    }
