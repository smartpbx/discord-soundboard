...
async function _httpHumanize(text, voiceName) {
    ...
    const body = {
        ...
        temperature: 0.4,
        // Headroom for Qwen3 in case thinking partially leaks despite the
        // chat_template_kwargs flag. 1024 min covers ~700 chars of actual
        // output plus any accidental reasoning tokens.
        max_tokens: Math.max(1024, Math.ceil(text.length * 3)),
        // Qwen3-family models default to thinking mode (chain-of-thought before
        // answer) which eats 1000–3000 tokens on a trivial rewrite — our
        // max_tokens cap then truncates the answer to empty. Explicitly
        // disable it via the Ollama chat_template_kwargs hook. Ignored by
        // non-Qwen3 backends, safe everywhere.
    ...
    if (typeof content !== 'string') {
        return { _error: 'no choices[0].message.content in response' };
    }
    // Qwen3 sometimes leaks the final answer into .reasoning and leaves
    // .content empty (especially when it ignores chat_template_kwargs). If
    // content is empty, try to salvage from reasoning by taking the last
    // non-empty line after any </think>-style delimiter.
    if (!content.trim() && typeof reasoning === 'string' && reasoning.trim()) {
        const afterThink = reasoning.split(/<\/think>|<\/thinking>|^final answer:?/im).pop();
        const lines = afterThink.split(/\n+/).map(l => l.trim()).filter(Boolean);
        if (lines.length) content = lines[lines.length - 1];
    }
    if (!content.trim()) {
        return { _error: `empty content (finish=${finish}, has_reasoning=${!!reasoning})` };
    }
