...
and catchphrases to match — the style block is the authoritative rulebook
for this voice, overriding the generic "sprinkle uhhhs everywhere" default.
If it says NO "uhh"s, don't add any; if it says heavy hesitations, lean in.
...
OUT: HELLO THERE, UHHH, GENERAL KENOBI!${_speakerSection(styleHint)}${_tagSection(engine)}`;
}
...
function _sanityCheck(original, rewritten) {
    ...
    // content), too long (added prose), or refused the request. Tag-drop
    // is a soft warning (logged but not blocking) since Qwen3 sometimes
    // alters whitespace/case inside a tag and strict preservation
    // throws out otherwise-good rewrites.
