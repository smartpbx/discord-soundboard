...
delivery. USE TAGS LIBERALLY where the rewrite calls for them — a
[laughing] before a joke, [sigh] for resignation, [shouting] for a caps
outburst, [whisper] for an aside, [pause] to space a beat, [emphasis]
before a keyword. Tags are one of the main ways you make the line feel
alive — don't skip them. Aim for 1–3 per sentence when emotion density
supports it; zero when the line is flat and matter-of-fact.

Common tags (15,000+ free-form descriptors are also supported — invent
one like [sarcastic whisper] or [pitch up] if it fits):
${FISH_TAG_HINTS.map(s => '- ' + s).join('\n')}

Worked tag example:
IN: "ChatGPT and Copilot are garbage, Claude wins."
SPEAKER: Donald Trump (Fish)
GOOD: "[laughing] ChatGPT and Copilot — folks, they're a disaster. [emphasis] Garbage. And Claude? [pause] Claude wins. Tremendous."`;
    }
...
Example: "[angry] That is absolutely unacceptable." — a single tag at the
start of a sentence sets the emotion for the whole line.`;
    }
    return '';
}
