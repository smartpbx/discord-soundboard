...
function buildPrompt(input) {
    const targetEngine = input.target_engine || 'rvc';
    if (targetEngine !== 'rvc') return buildLitePrompt(input, targetEngine);
    const lines = [
...
// Lite prompt for non-RVC engines (chatterbox / fish / gptsovits). The agent
// doesn't run the heavy 6-phase pipeline — it just finds good audio, picks a
// 5–15 s window, and invokes lite_voice_deploy.py on CT 110 once. Much cheaper
// (no GPU training, no diarization) and turnaround is minutes instead of hours.
function buildLitePrompt(input, engine) {
    const engineLabel = engine === 'fish' ? 'Fish-Speech v2 (S2 Pro)' : engine === 'gptsovits' ? 'GPT-SoVITS' : 'Chatterbox';
    const lines = [
        `Add a **${engineLabel}** voice for **${input.name}** (voice_id: \`${input.voice_id}\`).`,
        ``,
        `Group: ${input.group}`,
        `Gender: ${input.gender}`,
        `Target engine: ${engine}`,
        ``,
        `These engines are zero-shot — they don't need training, just one clean reference clip (5–15 s).`,
        `Your job: source good audio, pick the cleanest contiguous window, and deploy it via the lite-voice-deploy script on CT 110.`,
        ``,
    ];
    if (input.urls && input.urls.length) {
        lines.push(`Source URLs supplied:`);
        input.urls.forEach(u => lines.push(`  - ${u}`));
        lines.push(``, `Use the FIRST URL that yields a clean 5–15 s solo speech window. Probe with yt-dlp + ffprobe before downloading the whole thing.`);
    } else {
        lines.push(`No source URL supplied — find one yourself. Pick podcast/interview/monologue clips, NOT music videos or live concerts.`);
    }
    if (input.notes) {
        lines.push(``, `Additional notes from the requester: ${input.notes}`);
    }
    lines.push(
        ...
        `## Procedure`,
        ``,
        `1. **Pick a window.** Identify a [start_sec, end_sec] range (5–15 s) of clean solo target speech inside the chosen URL. If the audio has music underlay, skip the URL — Whisper transcripts get garbled.`,
        ``,
        `2. **Deploy in one shot** by SSH-invoking the lite deploy script on CT 110:`,
        `   \`\`\``,
        `   ssh root@10.10.10.72 "/opt/GPT-SoVITS/.venv/bin/python /opt/discord-soundboard/tts-server/tools/lite_voice_deploy.py \\`,
        `       --voice-id ${input.voice_id} --name '${(input.name || '').replace(/'/g, "'\"'\"'")}' --group ${input.group} --gender ${input.gender} \\`,
        `       --engine ${engine} \\`,
        `       --source-url <CHOSEN_URL> --start <START_SEC> --end <END_SEC>"`,
        `   \`\`\``,
        ``,
        `   The script downloads, ffmpeg-trims, Whisper-transcribes, writes models/${engine}/${input.voice_id}/{reference.wav,metadata.json}, and busts the engine cache. Watch its newline-delimited JSON output for the 'done' event.`,
        ``,
        `3. **Audition.** Tell the operator the voice is live and which test phrase you'd recommend (e.g. "Hello, my name is ${input.name}.")`,
        ``,
        `## Hard rules`,
        ``,
        `- Do **not** run the RVC pipeline scripts. This is a lite deploy — no 05_train.py, no Applio.`,
        `- If the chosen window is bad (wrong speaker, music, unclear audio), pick a different range or a different URL and re-run lite_voice_deploy.py with the same voice_id (it overwrites idempotently).`,
        `- Stop and report if 3 different windows in a row produce empty/garbled Whisper output — the source is probably unusable.`,
        `- Total token budget should stay under 30k input + 5k output for a lite deploy.`,
    );
    return lines.join('\n');
}
...
function startJob(input) {
    ...
    const cleanInput = {
        ...
        // 'rvc' = full 6-phase celebrity training; 'fish'/'gptsovits'/'chatterbox'
        // = lite deploy (just find audio + drop a 5-15s ref). Default rvc for
        // backward compat with older callers that omit the field.
        target_engine: ['rvc', 'fish', 'gptsovits', 'chatterbox'].includes(input.target_engine) ? input.target_engine : 'rvc',
    };
...
function _spawnQueuedJob(meta, resumePrompt) {
    ...
    // Re-read .env every spawn so a token rotation doesn't require a service
    // restart, and so the spawn doesn't depend on whatever parent state
    // process.env happens to be in. systemd's EnvironmentFile occasionally
    // misses fields on respawn — explicit re-read removes the foot-gun.
    const _spawnEnv = { ...process.env };
    try {
        const envPath = path.join(__dirname, '..', '.env');
        if (fs.existsSync(envPath)) {
            const parsed = require('dotenv').parse(fs.readFileSync(envPath));
            for (const [k, v] of Object.entries(parsed)) {
                if (v != null && v !== '') _spawnEnv[k] = v;
            }
        }
    } catch (e) {
        console.warn('[voice-trainer] could not re-read .env:', e.message);
    }
    if (!_spawnEnv.CLAUDE_CODE_OAUTH_TOKEN) {
        console.error('[voice-trainer] CLAUDE_CODE_OAUTH_TOKEN missing from spawn env; Claude will refuse to run.');
    }
...
function deleteJob(jobId) {
    ...
    // Refuse to nuke a running job from under its feet. Caller should cancel first.
    if (meta.status === 'running' && !meta.remote_only && meta.pid && isPidAlive(meta.pid)) {
        throw Object.assign(new Error('Job is still running — cancel it first, then delete'), { status: 409 });
    }
    const dir = path.join(JOBS_DIR, jobId);
    if (fs.existsSync(dir)) {
        try { fs.rmSync(dir, { recursive: true, force: true }); }
        catch (e) { throw Object.assign(new Error('Failed to delete job dir: ' + e.message), { status: 500 }); }
    }
    return { id: jobId, voice_id: meta.input && meta.input.voice_id, deleted: true };
}
...
function adoptOrphan(jobId) {
    ...
    saveJob(meta);
    // Append a synthetic event so the UI's text log shows context for the
    // user — otherwise the box looks frozen at the moment Claude died.
    try {
        const evt = {
            type: 'assistant',
            message: {
                content: [{
                    type: 'text',
                    text: `↳ Orphan adopted at ${new Date().toLocaleString()}. Tracking CT 110 train.log directly (no Claude session). When training finishes, run Phase 6 deploy manually.`,
                }],
            },
        };
        fs.appendFileSync(jobEventsPath(jobId), JSON.stringify(evt) + '\n');
    } catch (e) { console.warn('[voice-trainer] adopt: failed to append event:', e.message); }
    return meta;
}
...
function fetchRemoteTrainLog(jobId, remoteJobDir) {
    ...
    try {
        ...
        if (text) _trainLogCache.set(jobId, { at: now, dir: remoteJobDir, text });
        return text || (cached && cached.text) || '';
    } catch {
        // Network blip / SSH hiccup: keep returning the last known good tail so
        // the UI's progress bar / ETA don't visibly snap back to "no data" and
        // then forward again on the next successful poll.
