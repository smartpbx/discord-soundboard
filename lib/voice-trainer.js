...
function tickScheduler() {
    const jobs = listJobs();
    const runningIds = jobs.filter(j => j.status === 'running' && isPidAlive(j.pid));
    // Reap orphans — pid gone, meta still says running
    for (const j of jobs) {
        if (j.status === 'running' && (!j.pid || !isPidAlive(j.pid))) {
