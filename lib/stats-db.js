...
// NORMAL pairs naturally with WAL — fsync only on checkpoint instead of on
// every commit. The plays log is best-effort (a crash during a single
// insert is tolerable), so FULL durability costs more than it saves.
db.pragma('synchronous = NORMAL');
// Force a checkpoint after ~1000 pages so .db-wal doesn't grow unbounded.
// In prod the WAL had drifted to 4 MB without this.
...
db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS voice_trigger_events (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        at INTEGER NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_vte_at ON voice_trigger_events(at DESC);
    ...
    CREATE INDEX IF NOT EXISTS idx_vote_events_at ON vote_events(at DESC);
`);

// Migrate: add voters_json column on existing installs. ALTER TABLE ADD
// COLUMN throws if the column exists, so we wrap in try/catch.
try { db.exec("ALTER TABLE vote_events ADD COLUMN voters_json TEXT"); } catch {}
...
const stmtInsertVoiceTrigger = db.prepare(`
    INSERT INTO voice_trigger_events (at, speaker_user_id, trigger_id, phrase, transcript, sound_filename, status)
    VALUES (@at, @speaker_user_id, @trigger_id, @phrase, @transcript, @sound_filename, @status)
`);
const stmtListVoiceTriggers = db.prepare(`
    SELECT id, at, speaker_user_id, trigger_id, phrase, transcript, sound_filename, status
    FROM voice_trigger_events
    ORDER BY at DESC
    LIMIT @limit
`);
function recordVoiceTriggerEvent(entry) {
    try {
        stmtInsertVoiceTrigger.run({
            at: entry.when ?? Date.now(),
            speaker_user_id: entry.speakerUserId ?? null,
            trigger_id: entry.triggerId ?? null,
            phrase: entry.phrase ?? null,
            transcript: entry.transcript ?? null,
            sound_filename: entry.soundFilename ?? null,
            status: entry.status ?? null,
        });
    } catch (err) {
        console.error('[stats-db] recordVoiceTriggerEvent failed:', err.message);
    }
}
function listVoiceTriggerEvents(limit = 200) {
    try {
        return stmtListVoiceTriggers.all({ limit: Math.max(1, Math.min(2000, Number(limit) || 200)) }).map(r => ({
            when: r.at,
        ...
        console.error('[stats-db] listVoiceTriggerEvents failed:', err.message);
        return [];
    }
}

const stmtInsertVoteEvent = db.prepare(`
    INSERT INTO vote_events (at, type, target_user_id, target_username, target_display_name,
                             initiator_user_id, initiator_username, initiator_display_name,
                             yes, no, eligible, outcome, reason, minutes, voters_json)
    VALUES (@at, @type, @target_user_id, @target_username, @target_display_name,
            @initiator_user_id, @initiator_username, @initiator_display_name,
            @yes, @no, @eligible, @outcome, @reason, @minutes, @voters_json)
`);
const stmtListVoteEvents = db.prepare(`
    SELECT id, at, type, target_user_id, target_username, target_display_name,
           initiator_user_id, initiator_username, initiator_display_name,
           yes, no, eligible, outcome, reason, minutes, voters_json
    FROM vote_events
    ORDER BY at DESC
...
function recordVoteEvent(entry) {
    try {
        stmtInsertVoteEvent.run({
            at: entry.when ?? Date.now(),
            type: entry.type || 'kick',
            target_user_id: entry.targetUserId ?? null,
            target_username: entry.targetUsername ?? null,
            target_display_name: entry.targetDisplayName ?? null,
            initiator_user_id: entry.initiatorUserId ?? null,
            initiator_username: entry.initiatorUsername ?? null,
            initiator_display_name: entry.initiatorDisplayName ?? null,
            yes: entry.yes ?? 0,
            no: entry.no ?? 0,
            ...
            voters_json: Array.isArray(entry.voters) ? JSON.stringify(entry.voters) : null,
        });
    } catch (err) {
        console.error('[stats-db] recordVoteEvent failed:', err.message);
    }
}
function listVoteEvents(limit = 200) {
    try {
        return stmtListVoteEvents.all({ limit: Math.max(1, Math.min(2000, Number(limit) || 200)) }).map(r => {
            let voters = null;
            if (r.voters_json) { try { voters = JSON.parse(r.voters_json); } catch {} }
            return {
                when: r.at,
                type: r.type,
                ...
                initiatorUsername: r.initiator_username,
                initiatorDisplayName: r.initiator_display_name,
                yes: r.yes,
                no: r.no,
                eligible: r.eligible,
                outcome: r.outcome,
        ...
        console.error('[stats-db] listVoteEvents failed:', err.message);
        return [];
    }
}
...
module.exports = {
    ...
    recordVoiceTriggerEvent,
    listVoiceTriggerEvents,
    recordVoteEvent,
    listVoteEvents,
    close,
    _dbPath: DB_PATH,
